...
모든 필드가 선택이다. `law_query`가 있으면 해당 법령만, 없으면 시행일 범위로
검색한다. 범위도 없으면 최근 180일이 기본이다.

**즉시 202와 `analysis_id`를 반환하고 분석은 백그라운드에서 돈다.** 분석 1건은
법령 조회와 조문별 LLM 호출로 수십 초가 걸린다(실측 16초). 동기 응답으로 묶으면
NFR-006(장시간 무응답 UI 금지)을 위반한다.
...
RAG 미연결 상태에서는 `reference_evidence`가 빈 배열이다. 이는 오류가 아니라
정상 상태이며, 모델이 없는 출처를 지어내지 않았다는 뜻이다 (BR-005).
...
| RAG 참고자료 | 항상 빈 배열 | W5 |
| 보류 재판정 (FR-008) | 도메인 모델만 존재 | W3~W4 |
