...
# '정하다/규정하다'의 한국어 활용형을 포괄한다 (정한다·정하는·정할·규정한다 …).
# 어간만 쓰면 '정하'가 '정한다'에 매치되지 않아 위임을 통째로 놓친다. 이 함정을
# 두 번 밟았으므로(diff 엔진, 시행령 페어링) 공개 상수로 두고 같이 쓴다.
DELEGATION_VERB = r"(?:정하|정한|정할|정함|정해|규정)"
